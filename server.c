...
void start_game() {
    ...
    srand(time(NULL));
    target_number = rand() % 20 + 1; // 1 ile 20 arasında rastgele sayı
...
void process_guess(SOCKET client_socket, int guess) {
    ...
    if (guess == target_number) {
        snprintf(response, sizeof(response), "Tebrikler Dogru Sayi: %d!\n", target_number);
        send(client_socket, response, strlen(response), 0);
        start_game();  // Doğru tahminden sonra oyunu yeniden başlatın
    } else {
        attempts_left--;
        if (attempts_left > 0) {
            snprintf(response, sizeof(response), "Yanlis Tahmin. %d Hakkiniz kaldi. \n", attempts_left);
            send(client_socket, response, strlen(response), 0);
        } else {
            snprintf(response, sizeof(response), "Oyun Bitti! Doğru Sayi: %d.\n", target_number);
            send(client_socket, response, strlen(response), 0);
